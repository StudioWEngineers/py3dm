...
# Parse the header file <py3dm/bindings/bindings.h> and print the version.
def get_version(root: str) -> None:
    major = ''
    minor = ''
    patch = ''
    dev = ''
    with open(os.path.join(root, "src/bindings/bindings.h"), 'r') as f:
        for line in f:
            if m := re.match(r'#define PY3DM_VERSION_(.+)', line):
...
def write_version(root: str, semver: str) -> None:
    ...
    with open(os.path.join(root, "src/bindings/bindings.h"), "r+") as f:
        contents = f.read()
        contents = re.sub(r'#define PY3DM_VERSION_MAJOR\s+[0-9]+',
                          r'#define PY3DM_VERSION_MAJOR ' + str(major),
                          contents, count=1)
        contents = re.sub(r'#define PY3DM_VERSION_MINOR\s+[0-9]+',
                          r'#define PY3DM_VERSION_MINOR ' + str(minor),
                          contents, count=1)
        contents = re.sub(r'#define PY3DM_VERSION_PATCH\s+[0-9]+',
                          r'#define PY3DM_VERSION_PATCH ' + str(patch),
                          contents, count=1)
        contents = re.sub(r'#define PY3DM_VERSION_DEV\s+[0-9]+',
                          r'#define PY3DM_VERSION_DEV   ' + str(dev),
                          contents, count=1)
        f.seek(0)
        f.truncate()
        f.write(contents)
    ...
    with open(os.path.join(root, "py3dm/__init__.py"), "r+") as f:
        contents = f.read()
