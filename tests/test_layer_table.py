...
class layer_tableTestSuite(TestCase):
    def setUp(self) -> None:
        self.model = Model()
    ...
    def test_add(self) -> None:
        ...
        layer_1_uuid = self.model.layer_table.add(layer1)
        layer_2_uuid = self.model.layer_table.add(layer2)

        with self.subTest(msg="Number of layers with using Count"):
            self.assertEqual(self.model.layer_table.count(), 2)

        with self.subTest(msg="Number of layers with using __len__"):
            self.assertEqual(len(self.model.layer_table), 2)

        with self.subTest(msg="Name of 1st layer"):
            self.assertEqual(
                self.model.layer_table.get(layer_1_uuid).name,  # type: ignore
                "Layer A"
            )

        with self.subTest(msg="Name of 2nd layer"):
            self.assertEqual(
                self.model.layer_table.get(layer_2_uuid).name,  # type: ignore
                "layer 2"
            )

        with self.subTest(msg="Color of 1st layer"):
            self.assertEqual(
                self.model.layer_table.get(0).color,  # type: ignore
                (255, 0, 255, 255)
            )

    def test_count(self) -> None:
        with self.subTest(msg="Number of layers for an empty model"):
            self.assertEqual(self.model.layer_table.count(), 0)
    ...
    def test_delete_by_name(self) -> None:
        ...
        self.model.layer_table.add(layer_1)
    ...
    def test_delete_by_uuid(self) -> None:
        ...
        layer_1_uuid = self.model.layer_table.add(layer_1)
        self.model.layer_table.add(layer_2)
    ...
    def test_get_by_index(self) -> None:
        ...
        rl_1 = self.model.layer_table.get(0)
        rl_2 = self.model.layer_table.get(1)

        with self.subTest(msg="Retrieving not existing layer"):
            self.assertIsNone(self.model.layer_table.get(3))
    ...
    def test_get_by_name(self) -> None:
        ...
        rl_1 = self.model.layer_table.get("Layer A")
        rl_2 = self.model.layer_table.get("layer 2")

        with self.subTest(msg="Retrieving not existing layer"):
            self.assertIsNone(self.model.layer_table.get("dummy"))
    ...
    def test_get_by_uuid(self) -> None:
        ...
        layer_1_uuid = self.model.layer_table.add(layer_1)
        layer_2_uuid = self.model.layer_table.add(layer_2)

        rl_1 = self.model.layer_table.get(layer_1_uuid)
        rl_2 = self.model.layer_table.get(layer_2_uuid)

        with self.subTest(msg="Retrieving not existing layer"):
            self.assertIsNone(self.model.layer_table.get(UUID(int=0)))
    ...
    def test_get_index(self) -> None:
        ...
        with self.subTest(msg="Retrieving index of not existing layer"):
            self.assertEqual(self.model.layer_table.get_index(""), -2147483647)

        with self.subTest(msg="Index of 1st layer"):
            self.assertEqual(self.model.layer_table.get_index("Layer 01"), 0)

        with self.subTest(msg="Index of 2nd layer"):
            self.assertEqual(self.model.layer_table.get_index("Layer 02"), 1)

    def test_get_uuid(self) -> None:
        layer_1_uuid = self.model.layer_table.add(Layer())
        layer_2_uuid = self.model.layer_table.add(Layer())

        with self.subTest(msg="Retrieving uuid of not existing layer"):
            self.assertEqual(
                self.model.layer_table.get_uuid("Not existing"),
                UUID(int=0)
            )

        with self.subTest(msg="Index of 1st layer"):
            self.assertEqual(
                self.model.layer_table.get_uuid("Layer 01"),
                layer_1_uuid
            )

        with self.subTest(msg="Index of 2nd layer"):
            self.assertEqual(
                self.model.layer_table.get_uuid("Layer 02"),
                layer_2_uuid
            )
    ...
    def test_has(self) -> None:
        ...
        with self.subTest(msg="Not existing layer"):
            self.assertFalse(self.model.layer_table.has("Not existing"))

        with self.subTest(msg="1st layer"):
            self.assertTrue(self.model.layer_table.has("Layer 01"))

    def test_max_index(self) -> None:
        with self.subTest(msg="max_index before adding layers to model"):
            self.assertEqual(self.model.layer_table.max_index(), 0)
        ...
        with self.subTest(msg="max_index after add delete layers to model"):
            self.assertEqual(self.model.layer_table.max_index(), 3)


class Nestedlayer_tableTestSuite(TestCase):
    def test_nested_layer(self) -> None:
        model = Model()
        model.layer_table.add(Layer())
        parent_layer_uuid = model.layer_table.get_uuid("layer 01")

        child_layer = Layer()
        child_layer.parent_uuid = parent_layer_uuid
        child_layer.set_name("my child layer")

        model.layer_table.add(child_layer)
        child_uuid = model.layer_table.get_uuid("my child layer")

        self.assertEqual(
            model.layer_table.get(child_uuid).parent_uuid,  # type: ignore
            parent_layer_uuid
        )


class layer_tableTeIteratorstSuite(TestCase):
